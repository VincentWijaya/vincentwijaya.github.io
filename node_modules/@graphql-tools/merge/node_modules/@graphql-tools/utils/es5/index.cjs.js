...
function transformInputValue(type, value, transformer) {
    ...
    if (graphql.isLeafType(nullableType)) {
        ...
        Object.keys(value).forEach(function (key) {
            ...
            if (field != null) {
                newValue_1[key] = transformInputValue(field.type, value[key], transformer);
            }
        });
        return newValue_1;
    }
...
function filterRootFields(type, operation, rootFieldFilter) {
    if (rootFieldFilter) {
        var config_1 = type.toConfig();
        Object.keys(config_1.fields).forEach(function (fieldName) {
            if (!rootFieldFilter(operation, fieldName, config_1.fields[fieldName])) {
                delete config_1.fields[fieldName];
            }
        });
        return new graphql.GraphQLObjectType(config_1);
    }
    return type;
}
function filterElementFields(ElementConstructor, type, fieldFilter, argumentFilter) {
    ...
    if (fieldFilter || argumentFilter) {
        ...
        var config = type.toConfig();
        try {
            for (var _c = tslib.__values(Object.entries(config.fields)), _d = _c.next(); !_d.done; _d = _c.next()) {
                var _e = tslib.__read(_d.value, 2), fieldName = _e[0], field = _e[1];
                if (!fieldFilter(type.name, fieldName, config.fields[fieldName])) {
                    delete config.fields[fieldName];
                }
                else if (argumentFilter && 'args' in field) {
                    try {
                        for (var _f = (e_2 = void 0, tslib.__values(Object.keys(field.args))), _g = _f.next(); !_g.done; _g = _f.next()) {
                            var argName = _g.value;
